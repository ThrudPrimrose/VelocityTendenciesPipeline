...
def parse_signature(header_path: Path, fn_name_re: str) -> List[Tuple[str, bool, str]]:
    """Return list of (cpp_type_base, is_pointer, param_name) in signature order, skipping the state arg."""
    text = header_path.read_text()
    m = re.search(rf"void {fn_name_re}\((.*?)\);", text, re.S)
    if not m:
        raise SystemExit(f"couldn't find __program declaration matching {fn_name_re}")
    raw = m.group(1)
...
//
// Expands the 231-argument call list for the 4 velocity tendencies variants
// produced by generate_baselines.py. The generated SDFGs share an identical
// signature (unify_variant_signatures enforces this), so one argument list
// fits all four entry points.
...
#define VELOCITY_CALL_ARGS(global_data, p_diag, p_int, p_metrics, p_patch, p_prog, \\
...
def emit_macro(out_path: Path, exprs: List[str]):
    indent = " " * 4
    lines = []
    for i, e in enumerate(exprs):
        sep = "," if i + 1 < len(exprs) else ""
        lines.append(f"{indent}{e}{sep} \\")
    # Drop trailing backslash on the last line
    if lines:
...
def main():
    ...
    structs = parse_structs(Path("include") / args.header)
    params = parse_signature(Path(args.variant_header), args.fn)
    exprs = [map_param(structs, base, ptr, name) for base, ptr, name in params]
    emit_macro(Path(args.out), exprs)
    print(f"wrote {args.out} with {len(exprs)} arguments")
